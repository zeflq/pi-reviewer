...
- [x] **Keyboard shortcuts** — `j`/`k` next/prev comment (screen order, expands collapsed files), `a`/`r`/`d` accept/reject/discuss, `f` finish review, `?` cheatsheet overlay; keyboard icon button fixed bottom-left; click-outside deselects active comment
