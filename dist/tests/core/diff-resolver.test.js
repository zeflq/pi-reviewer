...
describe("resolveDiff", () => {
    ...
    it("auto-detects local base branch and uses merge-base diff", async () => {
        ...
        expect(execSyncMock).toHaveBeenNthCalledWith(2, "git symbolic-ref refs/remotes/origin/HEAD --short", {
            cwd: "/repo",
            encoding: "utf-8",
            stdio: ["pipe", "pipe", "pipe"],
        });
    ...
    it("throws when resolved diff is empty", async () => {
        ...
        await expect(resolveDiff({ diff: "HEAD~1", cwd: "/repo" })).rejects.toThrow("No changes found. Make sure you are on a feature branch with commits ahead of the base.");
    });
});
