...
describe("sendOutput", () => {
    ...
    it("posts to Issues API when no commitId is provided", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("https://api.github.com/repos/owner/repo/issues/42/comments", expect.objectContaining({
    ...
    it("posts to Reviews API with inline comments when commitId is provided", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("https://api.github.com/repos/owner/repo/pulls/42/reviews", expect.objectContaining({
            body: JSON.stringify({
                commit_id: "abc123",
                body: "Needs fixes",
                event: "COMMENT",
    ...
    it("falls back to Issues API when Reviews API returns 422", async () => {
        ...
        expect(fetchMock.mock.calls[1][0]).toBe("https://api.github.com/repos/owner/repo/issues/42/comments");
    });
    ...
    it("writes formatted review to pi-review.md for file target", async () => {
        ...
        expect(content).toBe("== Review Summary ==\nPlease address comments\n\n== Inline Comments ==\n🟡 src/a.ts:7 (RIGHT)\n🟡 Handle undefined");
